...
function bindAddShipment() {
    if (!shipmentForm || !trackingInput || !destinationInput)
        return;
    fromEvent(shipmentForm, "submit")
        ...
        .subscribe((shipment) => {
        if (!shipment || !trackingInput || !destinationInput)
            return;
...
function bindInputValidation() {
    if (!trackingInput || !destinationInput)
        return;
    fromEvent(trackingInput, "input")
        .pipe(debounceTime(300), map((e) => e.target.value.trim()))
        .subscribe((value) => {
        const isValid = value.length >= 8 || value.length === 0;
        trackingInput.classList.toggle("invalid", !isValid && value.length > 0);
    });
}
function bindStatusScrollSelection() {
    if (!shipmentList)
        return;
    fromEvent(shipmentList, "wheel")
        .pipe(filter((event) => {
...
function bindClearAll() {
    ...
    fromEvent(clearAllBtn, "click")
        .subscribe(() => {
        if (shipments$.getValue().length === 0)
            return;
...
function bindFilters() {
    ...
    }), filter((value) => value === "all" || value === "pending" || value === "in-transit" || value === "delivered"))
        .subscribe((filter) => {
...
function bindListActions() {
    ...
    fromEvent(shipmentList, "click")
        ...
        if (payload.action === "status" && payload.status) {
            const next = current.map((s) => s.id === payload.id
...
function bindRendering() {
    if (!shipmentList || !statsInfo)
        return;
    const visibleShipments$ = combineLatest([shipments$, filter$]).pipe(map(([shipments, filter]) => {
        let visible = shipments;
        if (filter === "pending")
            visible = shipments.filter((s) => s.status === "pending");
        if (filter === "in-transit")
            visible = shipments.filter((s) => s.status === "in-transit");
        if (filter === "delivered")
            visible = shipments.filter((s) => s.status === "delivered");
